...
public class Level1Boss extends Enemy {
	...
	public Level1Boss(TileMap tm, Player p, ArrayList<Enemy> enemies, ArrayList<Explosion> explosions) {
		...
		try {
			BufferedImage spritesheet = ImageIO.read(getClass().getResourceAsStream("/Sprites/Enemies/Boss1.gif"));
	...
	public void update() {
		...
		if (health <= maxHealth / 2) {
			...
			double position = ticks / 32;
			shield[0].setPosition(x + 30 * Math.sin(position), y + 30 * Math.cos(position));
			position += 3.1415;
			shield[1].setPosition(x + 30 * Math.sin(position), y + 30 * Math.cos(position));
		}
		...
		if (finalAttack) {
			...
			if (stepCount >= 90 && stepCount % 30 == 0) {
				...
				darkEnergy.setVector(3 * Math.sin(stepCount / 32), 3 * Math.cos(stepCount / 32));
		...
		if (steps[step] == 0) {
			...
			if (stepCount % 60 == 0) {
				DarkEnergy darkEnergy = new DarkEnergy(tileMap);
				darkEnergy.setType(DarkEnergy.GRAVITY);
				darkEnergy.setPosition(x, y);
				int direction = Math.random() < 0.5 ? 1 : -1;
				darkEnergy.setVector(direction, 0);
				enemies.add(darkEnergy);
			}
			if (stepCount == 559) {
				step++;
		...
		// floor sweep
			...
			if (stepCount == 60) {
				if (player.get_x() > tileMap.getWidth() / 2) {
					x = 30;
